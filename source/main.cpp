...
int main(int argc, char* argv[])
{
    ...
    if(argc<5 || argc>6){
        std::cout << "Error in calling moveDEM!" << std::endl
                  << "Usage: " << std::endl
                  << "./hu_code topDisplacement(m) bottomDisplacement(m) numberOfCalls dem_inputs [numThreads]..." << std::endl;
    }

    if (argc == 6) {
        dem::NUM_THREADS = atoi(argv[5]);
        std::cout << "command line: " << argv[0] << " " << argv[1] << " " << argv[2] << " " << argv[3] << " " << argv[4] << " " << argv[5] << std::endl;
    }
